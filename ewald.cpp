...
void Ewald::compute(int eflag, int vflag)
{
  ...
  int multi_int=0;
  int ifix, tag_particle;
  ifix = modify->multi_check();
  Fix_MULTI * multi_ptr;
  if (ifix != -1){
    multi_ptr = dynamic_cast <Fix_MULTI *> (modify->fix[ifix]);
    tag_particle=multi_ptr->tag_particle; 
    multi_ptr->force_ew_write=1;
  }
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    if (ifix != -1  && i==tag_particle){
        multi_ptr->force_ew[0]=qscale * q[i]*ek[i][0];
        multi_ptr->force_ew[1]=qscale * q[i]*ek[i][1];
        multi_ptr->force_ew[2]=qscale * q[i]*ek[i][2];
    }
  }
