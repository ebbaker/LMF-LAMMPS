...
Output::Output(LAMMPS *lmp) : Pointers(lmp)
{
  ...
  lmf_opened=0; lmf_slab_opened=0;
...
void Output::write_slab(bigint ntimestep){
    int nfix=modify->nfix;
    int lmf_slab=0;
    int ifix;

    ifix = modify->lmf_slab_check();
    if (ifix == -1) error->all(FLERR,"Could not find LMF fix");
    else lmf_slab=ifix;

    Fix_LMF_slab * slab_ptr = dynamic_cast <Fix_LMF_slab *> (modify->fix[lmf_slab]);

    int n_tot = slab_ptr->n_tot;
    double gs = slab_ptr->grid_spac;
    double * charge_dens = slab_ptr->charge_dens;
    double lo = slab_ptr->lmf_lo;
    double sigma = slab_ptr->lmf_sigma;
    double * sc = slab_ptr->smooth_slab;
    double * scw = slab_ptr->smooth_slab_wall;
    double * LMF_slab = slab_ptr->LMF_slab; 
    double grid_inv_sq = slab_ptr->grid_inv_sq;
    double pref = force->qqr2e/force->qe2f*4.0*3.14159;
    bool check_charge = slab_ptr->check_charge;

    int my_rank;
    MPI_Comm_rank(universe->uworld,&my_rank);
    if(!lmf_slab_opened && my_rank==0){
        lmf_slab_file = fopen(slab_ptr->lmf_slab_file_name, "w");
        lmf_slab_opened=1;
        printf("LMF slab file opened - %s \n", slab_ptr->lmf_slab_file_name);
    }
  
    if(my_rank==0){

        fprintf(lmf_slab_file, "Timestep %lld %d %.6f %.6f %.6f %.6f %.6f \n", ntimestep, n_tot, gs, sigma, thermo->return_temp(), slab_ptr->E_Field_Variable, (domain->boxhi[0]-domain->boxlo[0])*(domain->boxhi[1]-domain->boxlo[1]));
        for(int n_2 = 0; n_2 < n_tot; n_2++){
            fprintf(lmf_slab_file, "%d ", n_2);
            fprintf(lmf_slab_file,"%g ", float(n_2)*gs+lo);
            fprintf(lmf_slab_file,"%g ", charge_dens[n_2]);
            fprintf(lmf_slab_file,"%g %g ", slab_ptr->o_dens_g[n_2], slab_ptr->h_dens_g[n_2]);
            if(check_charge==0)fprintf(lmf_slab_file,"%g %g \n", LMF_slab[n_2], sc[n_2]);
            else fprintf(lmf_slab_file,"%g %g %g \n", LMF_slab[n_2], sc[n_2], scw[n_2]);
        }		
    }
    fflush(lmf_slab_file);
  
}


void Output::write_lmf(bigint ntimestep){
    int nfix=modify->nfix;
    int lmf=0;
    int ifix;

    ifix = modify->lmf_check();
    if (ifix == -1) error->all(FLERR,"Could not find LMF fix");
    else lmf=ifix;

    Fix_LMF * lmf_ptr = dynamic_cast <Fix_LMF *> (modify->fix[lmf]);

    int *n_tot = lmf_ptr->n_tot;
  ...
  https://mail.google.com/mail/u/0/#inbox  bool ensemble_flag=lmf_ptr->ensemble_flag;
    ...
    double *grid_inv_sq = lmf_ptr->grid_inv_sq;

    int my_rank;
    MPI_Comm_rank(universe->uworld,&my_rank);

    if(!lmf_opened && my_rank==0){
        lmf_file = fopen(lmf_ptr->lmf_file_name, "w");
        lmf_opened=1;
        printf("LMF file opened - %s \n", lmf_ptr->lmf_file_name);
    }

    if(my_rank==0){
    
        fprintf(lmf_file, "Timestep %lld %d %d %d %.6f %.6f %.6f %.6f %.6f %.6f \n", ntimestep, n_tot[0], n_tot[1], n_tot[2], gs[0], gs[1], gs[2], sigma, thermo->return_temp(), (domain->boxhi[0]-domain->boxlo[0])*(domain->boxhi[1]-domain->boxlo[1]));
        for(int n_0 = 0; n_0 < n_tot[0]; n_0++){
            for(int n_1 = 0; n_1 < n_tot[1]; n_1++){
                for(int n_2 = 0; n_2 < n_tot[2]; n_2++){
                    fprintf(lmf_file, "%d %d %d ", n_0,n_1,n_2);
                    fprintf(lmf_file,"%g %g %g ", float(n_0)*gs[0]+lo[0],float(n_1)*gs[1]+lo[1],float(n_2)*gs[2]+lo[2]);
                    fprintf(lmf_file,"%g %g ", LMF_potential->array[n_0][n_1][n_2], sc->array[n_0][n_1][n_2]);
                    if(ensemble_flag) fprintf(lmf_file,"%g %g ", LMF_potential->array_g[n_0][n_1][n_2], sc->array_g[n_0][n_1][n_2]); 
                    fprintf(lmf_file,"\n");
                }
