...
void Verlet::run(int n)
{
  ...
  for (int i = 0; i < n; i++) {
    ...
    if(lmf_slab!=-1){
        ...
        slab_ptr->LMF_update_slab();
        if(slab_ptr->lmf_slab_write != 0)
            if((ntimestep%slab_ptr->lmf_slab_write) == 0)
                output->write_slab(ntimestep);		
    }
    int lmf = modify->lmf_check();
    if(lmf!=-1){
        Fix_LMF * lmf_ptr = dynamic_cast <Fix_LMF *> (modify->fix[lmf]);
        lmf_ptr->LMF_update();
        if(lmf_ptr->lmf_write != -1)
            if((ntimestep%lmf_ptr->lmf_write) == 0)
                output->write_lmf(ntimestep);		
    }
    int multi = modify->multi_check();
    if(multi!=-1){
        Fix_MULTI * multi_ptr = dynamic_cast <Fix_MULTI *> (modify->fix[multi]);
        multi_ptr->MULTI_update();	
    }
  }
}
