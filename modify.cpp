...
int Modify::lmf_slab_check(){
  int ifix;
  for (ifix = 0; ifix < nfix; ifix++){
    if (strcmp("lmf_slab",fix[ifix]->style) == 0) break;
  }
  if (ifix == nfix) return(-1);
  else return(ifix);
}

int Modify::lmf_check(){
        int ifix;
  for (ifix = 0; ifix < nfix; ifix++){
    if (strcmp("lmf",fix[ifix]->style) == 0) break;
  }
  if (ifix == nfix) return(-1);
  else return(ifix);
}

int Modify::multi_check(){
        int ifix;
  for (ifix = 0; ifix < nfix; ifix++){
    if (strcmp("multi",fix[ifix]->style) == 0) break;
  }
  if (ifix == nfix) return(-1);
  else return(ifix);
}
