	...
	for (int i = 0; i < atom->nlocal; i++)
        if (mask[i] & groupbit) {
          for(int n_t=0; n_t<n_aut; n_t++){
                ...
                std::complex<double> imag(0.0,1.1);
                charge_autvals[num_lags-1][n_t]=charge_autvals[num_lags-1][n_t]+std::exp(std::complex<double>(2.0*3.1415*imag*pos_val))*std::complex<double>(q[i]);
                }
        }
}
