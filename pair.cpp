...
void Pair::init_tables(double cut_coul, double *cut_respa)
{
  ...
  double g_ewald;
  if (force->kspace != NULL) g_ewald = force->kspace->g_ewald;
  else if(g_ewald_trunc!=0.0){
    g_ewald=g_ewald_trunc;
  }
  else error->all(FLERR,"Pair style requires a KSpace style or g_ewald_trunc value");
